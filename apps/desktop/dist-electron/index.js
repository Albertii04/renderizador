...
const electronUpdater = /* @__PURE__ */ getDefaultExportFromCjs(mainExports);
var src = { exports: {} };
...
function startAutoUpdateLoop() {
  ...
  autoUpdater.checkForUpdatesAndNotify().catch((error2) => {
    log.warn("initial update check failed", error2);
  });
  if (pollHandle) clearInterval(pollHandle);
  pollHandle = setInterval(() => {
    autoUpdater.checkForUpdates().catch((error2) => {
      log.warn("hourly update check failed", error2);
    });
  }, HOUR_MS);
}
async function checkForUpdates() {
  try {
    ...
    return {
      status: (result == null ? void 0 : result.updateInfo.version) ? "available" : "not_available",
      version: result == null ? void 0 : result.updateInfo.version
    };
  } catch (error2) {
    return {
      status: "error",
      message: error2 instanceof Error ? error2.message : "Unknown update error"
    };
  }
}
...
async function readStationConfig() {
...
async function writeStationConfig(input) {
...
async function launchConfiguredBinary(binaryPath, args = []) {
  ...
  try {
    ...
    return {
      ok: false,
      message: error2 instanceof Error ? error2.message : "Unable to launch configured executable."
    };
  }
}
...
async function startMicrosoftAuth(authUrl, redirectTo) {
  ...
  return await new Promise((resolve2) => {
    ...
    void authWindow.loadURL(authUrl).catch((error2) => {
      finish({
        ok: false,
        message: error2 instanceof Error ? error2.message : "Unable to open Microsoft sign-in."
      });
    });
  });
}
if (!hasSingleInstanceLock) {
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.handle(
      "auth:microsoft",
      async (_event, input) => startMicrosoftAuth(input.authUrl, input.redirectTo)
    );
    ipcMain.handle(
      "rdp:connect",
      async (_event, input) => connectRdpInternal(input)
    );
